...
class Group {
  static #groups = {}; // Створюємо приватне статичне поле #groups для зберігання усіх створених груп. Має початкове значени: пустий об'єкт

  constructor(name) {
    this.name = name; // Створюмєо конструктор класу, який приймає назву групи як аргумент та ініціалізує поле this.name.
  }

  static create(name) {
    if (!this.#groups[name]) {
      this.#groups[name] = new Group(name); // Створюємо статичний метод create, який приймає назву групи name як аргумент.
    }
    return this.#groups[name]; // Повертаємо екземпляр групи з вказаною назвою.
  }
}

class Product {
  constructor(name, group) {
    this.name = name; // Створюємо конструктор класу, який приймає назву продукту name та групу group як аргументи та ініціалізує відповідні поля.
    this.group = group;
  }

  display() {
    console.log(`Продукт: ${this.name}, Група: ${this.group.name}`); // Метод display виводить інформацію про продукт в консоль
  }
}
